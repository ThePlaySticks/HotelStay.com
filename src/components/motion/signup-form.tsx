...
export function SignUpForm({
  ...
  description = "Join HotelStay to reserve suites and manage your journeys.",
  ...
  return (
    <form
      ...
      className={cn(
        "flex w-full max-w-md flex-col gap-5 rounded-3xl border border-stone-200/90 bg-white p-6 sm:p-8 shadow-xl",
        className,
        classNames?.root,
      )}
    >
      {title || description ? (
        <div className={cn("flex flex-col gap-1", classNames?.header)}>
          {title ? (
            <h2
              className={cn(
                "font-editorial text-2xl font-bold tracking-tight text-[#141413]",
                classNames?.title,
              )}
            >
              {title}
          ...
          {description ? (
            <p
              className={cn(
                "text-xs text-stone-500 leading-relaxed",
      ...
      <div className={cn("flex flex-col gap-1.5", classNames?.fields)}>
        <Input
          label="Full Name"
          autoComplete="name"
          ...
          <AnimatePresence initial={false}>
            {showStrength ? (
              ...
              >
                ...
                >
                  Password strength: <span className="font-semibold text-stone-700">{STRENGTH_LABELS[strength]}</span>
                </p>
              </motion.div>
            ) : null}
          </AnimatePresence>
        ...
        <AnimatePresence initial={false}>
          {shownError("terms") ? (
            <motion.p
              ...
              className="px-1 text-xs text-red-600 font-medium"
            >
              {shownError("terms")}
            </motion.p>
          ) : null}
      ...
      <AnimatePresence initial={false}>
        {errorMessage ? (
          <motion.p
            ...
            exit={reduce ? { opacity: 0 } : { opacity: 0, y: -4 }}
            transition={{ duration: 0.2 }}
            className="rounded-2xl border border-red-200 bg-red-50 px-3.5 py-2.5 text-xs text-red-700 font-medium leading-relaxed"
          >
            {errorMessage}
          </motion.p>
        ) : null}
      ...
      <StatefulButton
        ...
        className={cn("w-full mt-1", classNames?.submit)}
      >
        {submitLabel}
      </StatefulButton>
